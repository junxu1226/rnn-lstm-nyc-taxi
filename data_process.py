...
inputs_x0 = np.empty((nsamples, seq, 2), dtype='float32')
outputs_x0 = np.empty((nsamples, seq, 2), dtype='float32')
...
for i in range(nsamples):
    ...
    # eachPiece0 = np.random.rand(seq-1, 2) * 2 + 1
    eachPiece0 = np.random.uniform(0, .5, (3, 2)) * 2
    eachPiece1 = np.random.uniform(.5, 1, (7, 2)) * 2
    eachPiece = np.concatenate((eachPiece0, eachPiece1))
    # eachPiece = np.random.rand(seq, 2) * 2 + 1
    # fillEnd = np.ones((1, 2)) * 3
    # eachPiece = np.concatenate((eachPiece0, fillEnd))
    # eachPiece = eachPiece.swapaxes(0,1)
    # eachPiece = eachPiece.tolist()
    inputs_x0[i] = eachPiece
...
features_x0 = f.create_dataset('features_x0', inputs_x0.shape, dtype='float32')
# features_x1 = f.create_dataset('features_x1', (nsamples,), dtype=dtype)

targets_x0 = f.create_dataset('targets_x0', outputs_x0.shape, dtype='float32')
...
split_dict = {
    'train': {'features_x0': (0, nsamples_train), 'targets_x0': (0, nsamples_train)},
    'test': {'features_x0': (nsamples_train, nsamples), 'targets_x0': (nsamples_train, nsamples)}}
